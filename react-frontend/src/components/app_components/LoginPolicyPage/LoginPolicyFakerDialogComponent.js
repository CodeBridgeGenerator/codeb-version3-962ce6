...
import loginPolicyFakerFactory from "./loginPolicyFakerFactory";
...
const LoginPolicyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = loginPolicyFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("loginPolicy").create(elem));
...
export default connect(mapState, mapDispatch)(LoginPolicyFakerDialogComponent);
