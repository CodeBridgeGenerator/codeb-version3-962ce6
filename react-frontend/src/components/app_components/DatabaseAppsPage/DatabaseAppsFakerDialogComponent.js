...
import databaseAppsFakerFactory from "./databaseAppsFakerFactory";
...
const DatabaseAppsFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = databaseAppsFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("databaseApps").create(elem));
...
export default connect(mapState, mapDispatch)(DatabaseAppsFakerDialogComponent);
