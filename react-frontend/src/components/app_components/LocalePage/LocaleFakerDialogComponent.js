...
import localeFakerFactory from "./localeFakerFactory";
...
const LocaleFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = localeFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("locale").create(elem));
...
export default connect(mapState, mapDispatch)(LocaleFakerDialogComponent);
