...
import disasterRecoveryFakerFactory from "./disasterRecoveryFakerFactory";
...
const DisasterRecoveryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = disasterRecoveryFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("disasterRecovery").create(elem));
...
export default connect(mapState, mapDispatch)(DisasterRecoveryFakerDialogComponent);
