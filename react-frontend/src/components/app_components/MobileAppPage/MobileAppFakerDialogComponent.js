...
import mobileAppFakerFactory from "./mobileAppFakerFactory";
...
const MobileAppFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = mobileAppFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("mobileApp").create(elem));
...
export default connect(mapState, mapDispatch)(MobileAppFakerDialogComponent);
