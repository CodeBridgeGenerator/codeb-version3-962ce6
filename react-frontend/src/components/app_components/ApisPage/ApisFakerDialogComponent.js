...
import apisFakerFactory from "./apisFakerFactory";
...
const ApisFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = apisFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("apis").create(elem));
...
export default connect(mapState, mapDispatch)(ApisFakerDialogComponent);
