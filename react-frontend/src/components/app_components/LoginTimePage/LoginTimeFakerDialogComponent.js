...
import loginTimeFakerFactory from "./loginTimeFakerFactory";
...
const LoginTimeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = loginTimeFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("loginTime").create(elem));
...
export default connect(mapState, mapDispatch)(LoginTimeFakerDialogComponent);
