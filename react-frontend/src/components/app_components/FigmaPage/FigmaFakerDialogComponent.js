...
import figmaFakerFactory from "./figmaFakerFactory";
...
const FigmaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = figmaFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("figma").create(elem));
...
export default connect(mapState, mapDispatch)(FigmaFakerDialogComponent);
