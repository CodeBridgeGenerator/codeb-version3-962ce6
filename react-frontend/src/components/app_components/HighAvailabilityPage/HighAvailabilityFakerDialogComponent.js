...
import highAvailabilityFakerFactory from "./highAvailabilityFakerFactory";
...
const HighAvailabilityFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = highAvailabilityFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("highAvailability").create(elem));
...
export default connect(mapState, mapDispatch)(HighAvailabilityFakerDialogComponent);
