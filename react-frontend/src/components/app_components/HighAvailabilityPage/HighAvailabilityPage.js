...
import HighAvailabilityDatatable from "./HighAvailabilityDataTable";
import HighAvailabilityEditDialogComponent from "./HighAvailabilityEditDialogComponent";
import HighAvailabilityCreateDialogComponent from "./HighAvailabilityCreateDialogComponent";
import HighAvailabilityFakerDialogComponent from "./HighAvailabilityFakerDialogComponent";
import HighAvailabilitySeederDialogComponent from "./HighAvailabilitySeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const HighAvailabilityPage = (props) => {
    ...
    const filename = "highAvailability.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("highAvailability");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "High Availability", type: "error", message: error.message || "Failed get High Availability" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("highAvailability").remove(selectedEntityIndex);
            ...
            console.debug({ error });
            props.alert({ title: "High Availability", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("highAvailability").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/highAvailability/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="highAvailability-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="highAvailability-datatable">
                    <HighAvailabilityDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <HighAvailabilityEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <HighAvailabilityCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <HighAvailabilityFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <HighAvailabilitySeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(HighAvailabilityPage);
