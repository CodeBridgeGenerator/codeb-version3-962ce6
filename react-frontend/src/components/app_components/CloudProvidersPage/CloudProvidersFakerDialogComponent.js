...
import cloudProvidersFakerFactory from "./cloudProvidersFakerFactory";
...
const CloudProvidersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cloudProvidersFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cloudProviders").create(elem));
...
export default connect(mapState, mapDispatch)(CloudProvidersFakerDialogComponent);
