...
import pagesFakerFactory from "./pagesFakerFactory";
...
const PagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = pagesFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pages").create(elem));
...
export default connect(mapState, mapDispatch)(PagesFakerDialogComponent);
