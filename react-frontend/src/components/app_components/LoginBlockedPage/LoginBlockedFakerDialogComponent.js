...
import loginBlockedFakerFactory from "./loginBlockedFakerFactory";
...
const LoginBlockedFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = loginBlockedFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("loginBlocked").create(elem));
...
export default connect(mapState, mapDispatch)(LoginBlockedFakerDialogComponent);
