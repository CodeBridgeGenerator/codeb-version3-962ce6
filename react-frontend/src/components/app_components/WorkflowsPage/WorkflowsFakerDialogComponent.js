...
import workflowsFakerFactory from "./workflowsFakerFactory";
...
const WorkflowsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = workflowsFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workflows").create(elem));
...
export default connect(mapState, mapDispatch)(WorkflowsFakerDialogComponent);
