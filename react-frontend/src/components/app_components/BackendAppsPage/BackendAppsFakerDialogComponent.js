...
import backendAppsFakerFactory from "./backendAppsFakerFactory";
...
const BackendAppsFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = backendAppsFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("backendApps").create(elem));
...
export default connect(mapState, mapDispatch)(BackendAppsFakerDialogComponent);
