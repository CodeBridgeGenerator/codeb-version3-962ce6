...
const EnvironmentsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("environments");
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="environments-add-button"/>
...
/>            
      ...
      />
            ...
            <EnvironmentsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
