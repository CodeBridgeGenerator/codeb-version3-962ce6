...
import sharingFakerFactory from "./sharingFakerFactory";
...
const SharingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sharingFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sharing").create(elem));
...
export default connect(mapState, mapDispatch)(SharingFakerDialogComponent);
