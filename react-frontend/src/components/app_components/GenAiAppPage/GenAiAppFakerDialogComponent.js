...
import genAiAppFakerFactory from "./genAiAppFakerFactory";
...
const GenAiAppFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = genAiAppFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("genAiApp").create(elem));
...
export default connect(mapState, mapDispatch)(GenAiAppFakerDialogComponent);
