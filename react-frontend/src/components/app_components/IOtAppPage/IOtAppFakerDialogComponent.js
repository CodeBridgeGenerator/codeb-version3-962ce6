...
import iOtAppFakerFactory from "./iOtAppFakerFactory";
...
const IOtAppFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = iOtAppFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("iOtApp").create(elem));
...
export default connect(mapState, mapDispatch)(IOtAppFakerDialogComponent);
