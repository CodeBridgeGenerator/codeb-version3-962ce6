...
import themeFakerFactory from "./themeFakerFactory";
...
const ThemeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = themeFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("theme").create(elem));
...
export default connect(mapState, mapDispatch)(ThemeFakerDialogComponent);
