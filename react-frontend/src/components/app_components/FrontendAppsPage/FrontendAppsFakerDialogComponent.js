...
import frontendAppsFakerFactory from "./frontendAppsFakerFactory";
...
const FrontendAppsFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = frontendAppsFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("frontendApps").create(elem));
...
export default connect(mapState, mapDispatch)(FrontendAppsFakerDialogComponent);
