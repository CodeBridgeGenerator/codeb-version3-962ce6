...
import GitRepositoriesDatatable from "./GitRepositoriesDataTable";
import GitRepositoriesEditDialogComponent from "./GitRepositoriesEditDialogComponent";
import GitRepositoriesCreateDialogComponent from "./GitRepositoriesCreateDialogComponent";
import GitRepositoriesFakerDialogComponent from "./GitRepositoriesFakerDialogComponent";
import GitRepositoriesSeederDialogComponent from "./GitRepositoriesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const GitRepositoriesPage = (props) => {
    ...
    const filename = "gitRepositories.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("gitRepositories");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , projectId : urlParams.singleProjectsId ,environment : urlParams.singleEnvironmentsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Git Repositories", type: "error", message: error.message || "Failed get Git Repositories" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("gitRepositories").remove(selectedEntityIndex);
            ...
            console.debug({ error });
            props.alert({ title: "Git Repositories", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("gitRepositories").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/gitRepositories/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="gitRepositories-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="gitRepositories-datatable">
                    <GitRepositoriesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <GitRepositoriesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <GitRepositoriesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <GitRepositoriesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <GitRepositoriesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(GitRepositoriesPage);
