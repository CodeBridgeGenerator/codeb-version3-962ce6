...
import gitRepositoriesFakerFactory from "./gitRepositoriesFakerFactory";
...
const GitRepositoriesFakerDialogComponent = (props) => {
...
client.service("environments")
    ...
    const onRun = async () => {
        let fakeData = gitRepositoriesFakerFactory(props.user,fakerCount,projectIdIds,environmentIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("gitRepositories").create(elem));
...
export default connect(mapState, mapDispatch)(GitRepositoriesFakerDialogComponent);
