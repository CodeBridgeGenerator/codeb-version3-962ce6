...
import passwordPolicyFakerFactory from "./passwordPolicyFakerFactory";
...
const PasswordPolicyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = passwordPolicyFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("passwordPolicy").create(elem));
...
export default connect(mapState, mapDispatch)(PasswordPolicyFakerDialogComponent);
